...
int main() {
    ...
    for (int i = 0; i < files.size(); i ++ ) {
        std::cout << "Id " << files[i].get_id() << std::endl
                  << "Title " << files[i].get_title() << std::endl
                  << "mimeType " << files[i].get_mimeType() << std::endl << std::endl;
    }
    files.clear();
    ...
    while(true) {
        ...
        const std::vector<GFile>& items = filelist.get_items();
