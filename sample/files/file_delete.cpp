...
int main() {
    ...
    // First, insert a file to Google drive
    std::ifstream fin("file_delete.cpp");
    FileContent fc(fin, "text/plain");
    GFile file;
    file.set_title("file_delete.cpp");
    file.set_description("This is a sample source code of how to use allenbo/gdrive to delete a file on Google drive");
    file.set_mimeType("text/plain");
    file = service.files().Insert(&file, &fc).execute();
    fin.close();

    // Now, delete it permanently 
    service.files().Delete(file.get_id()).execute();
}
